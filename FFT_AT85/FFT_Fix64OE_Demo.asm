;16-bit Real-Time FFT Demo on an 8bit AVR (ATMega88) (As On Youtube, DigitalPhage Channel, http://www.youtube.com/watch?v=tCmaOb-VAEo )
;Craig Webster (IXIBA)
;Ver 30/11/2013
...
;There are 2 input buffers, while one is being FFT'd the other is filling with ADC samples.
;As long as the FFT finishs before the ADC buffer fills there will be no dropped data.
...
Buffer_WaitT:
	sbrc	flags,Toggle_Buf	;Alternate which buffer to do a 'Full' test on 1st
	rjmp	BW_tryB
...
;I * I = r23:r22 * r23:r22 = r11:r10:r9:r8
	...
	add	r12,r22			;Add to previous sample for averaging
	adc	r13,r23
...
FFT_Fixed64OE:		;FFT 64 Fixpoint, Odd Even Decomposition (Ver 26/5/2013)
				...
				;A Cooley–Tukey FFT derived from
...
;This is the Odd-Even Decompostion part
;From Digital Signal Processing, A Practical Guide for Engineers & Scientists. Smith
...
;Table offsets to generate the "Decimation in Time" of input samples
;Removes the need for a dedicated subroutinte to do the decimation
Bit_Flip_OffsetsOE:
...
FFT_Data_Real_AV:		;Averaging buffer for real samples
	.byte FFT_Size	;RI to R, N16 (First 16 and last 16 won't be used)
...
Fill_Offset:			;pointer to next ADC store pos in FFT_Data (Decimation offset)
	.byte	1
